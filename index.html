...
<head>
    ...
    <style>
        ...
        /* 検索バーのスタイル */
        .search-container { max-width: 500px; margin: 15px auto 0; padding: 0 20px; }
        .search-bar { width: 100%; padding: 12px 15px; border-radius: 25px; border: 1px solid #ddd; box-sizing: border-box; font-size: 16px; outline: none; }
        .search-bar:focus { border-color: var(--primary-blue); box-shadow: 0 0 5px rgba(0,120,212,0.2); }
        ...
        .version-info { margin-top: 40px; padding-top: 10px; border-top: 1px solid #eee; color: #aaa; font-size: 11px; text-align: center; }
    </style>
</head>
<body>
...
<script>
    ...
    function showPage(pageId) {
        ...
        // 設定ページ以外では検索バーを表示
        document.querySelector('.search-container').style.display = (pageId === 'settings') ? 'none' : 'block';
    }
    ...
    function render() {
        ...
        railData.forEach((item, index) => {
            ...
            const catList = document.getElementById('list-' + item.cat);
            if(catList) catList.insertAdjacentHTML('beforeend', html);
        });
    }

    function saveLocal() { localStorage.setItem('railData', JSON.stringify(railData)); }
    function clearAll() { if(confirm("全データを削除しますか？")) { railData = []; saveLocal(); render(); } }

    async function syncSave() {
        const btn = document.getElementById('btn-save');
        btn.innerText = "保存中..."; btn.disabled = true;
        try {
            await fetch(API_URL, { method: 'POST', body: JSON.stringify(railData) });
            alert("クラウドに保存しました");
        } catch(e) { alert("エラー: " + e); }
        btn.innerText = "現在の状態を保存 (送信)"; btn.disabled = false;
    }

    async function syncLoad() {
        try {
            const res = await fetch(API_URL);
            railData = await res.json();
            saveLocal(); render();
            alert("読み込み完了");
        } catch(e) { alert("受信失敗: " + e); }
    }

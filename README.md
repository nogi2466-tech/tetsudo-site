...
<head>
    ...
    <style>
        ...
        .search-container input { margin: 0; border: 1px solid #ddd; border-radius: 20px; padding: 10px 20px; width: 90%; max-width: 400px; font-size: 16px; }

        .tag { font-size: 10px; padding: 2px 6px; border-radius: 4px; margin-right: 5px; color: white; vertical-align: middle; text-transform: uppercase; }
        ...
        nav { background: white; padding: 10px 0; position: sticky; top: 55px; z-index: 100; border-top: 1px solid #eee; display: flex; justify-content: center; overflow-x: auto; white-space: nowrap; }
        ...
        .url-item { padding: 12px 0; border-bottom: 1px solid #f0f0f0; position: relative; }
        .url-info { display: flex; justify-content: space-between; align-items: center; }
        .url-info a { text-decoration: none; color: #333; flex-grow: 1; -webkit-user-select: text; user-select: text; }
        .url-info b { font-size: 16px; }
        ...
        .del-btn { background: none; border: none; color: #ff5252; cursor: pointer; font-size: 12px; padding: 5px; }
    </style>
</head>
<body>
    ...
    <section id="settings">
        ...
        <p style="font-size: 11px; color: #888; text-align: center;">※スクロール等による自動同期は行いません</p>
        <hr>
...
<script>
    ...
    function render() {
        ...
        const containers = {
            ...
            keio: document.getElementById('list-keio'),
            jr: document.getElementById('list-jr'),
            files: document.getElementById('list-files'),
        ...
        railData.forEach((item, index) => {
            ...
            const tagClass = item.cat.toUpperCase();
            const secretClass = item.secret ? 'is-secret' : '';
            const secretBadge = item.secret ? '<span class="secret-badge">秘密</span>' : '';
            const deleteBtn = isAdmin ? `<button class="del-btn" onclick="deleteItem(${index})">削除</button>` : '';

            const itemHtml = `
                <div class="url-item ${secretClass}">
                    <div class="url-info">
                        <a href="${item.url}" target="_blank">
                            <span class="tag ${tagClass}">${tagClass}</span>
                            <b>${item.title}</b>
                            ${secretBadge}
                        </a>
                        ${deleteBtn}
            ...
            containers.all.insertAdjacentHTML('beforeend', itemHtml);
            if (containers[item.cat]) {
                containers[item.cat].insertAdjacentHTML('beforeend', itemHtml);
            }
        });
    }
    ...
    // --- 完全手動の同期機能 ---
    function syncLoad() {
        if(!confirm("クラウドからデータを取得しますか？（現在の表示が更新されます）")) return;
        
        // ここにfetch(API_URL)...などの通信処理を記述
        // 現状は手動操作を確認するのみ
        console.log("手動同期完了");
        alert("手動同期を行いました（データがある場合はここに読み込み処理が入ります）");
        render();
    }

    function syncSave() {
        if(!isAdmin) return;
        alert("クラウドへ保存しました（送信完了）");
        saveLocal();
    }
